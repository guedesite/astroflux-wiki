...
function renderSystemPage(key, record, cache, routes) {
  ...
  const bodyRows = bodies.map(([bodyKey, body]) => {
    ...
    return `<tr><td>${entityLink("Bodies", bodyKey, cache, routes)}</td><td>${escapeHtml(valueLabel(body.type))}</td><td>${escapeHtml(valueLabel(body.level))}</td><td>${escapeHtml(valueLabel(body.landable))}</td><td>${escapeHtml(valueLabel(body.explorable))}</td><td>${escapeHtml(valueLabel(pos?.x))}, ${escapeHtml(valueLabel(pos?.y))}</td></tr>`;
  }).join("");
  ...
  <section class="content-grid">
    <article><h2>Overview</h2><div class="stat-grid">${statCard("Galaxy", record.galaxy)}${statCard("Status", status.label, status.access)}${statCard("Coordinates", `${valueLabel(record.x)}, ${valueLabel(record.y)}`)}${statCard("PvP disabled", record.pvpDisabled)}${statCard("Locations", bodies.length)}</div></article>
    ${statusNotice}
    ${warpLinks}
...
<div class="map-head">
  <div><h2>${compact ? "Galaxy Map" : "Interactive Galaxy Map"}</h2><p class="muted">Click a system to open its page. Destroyed or inaccessible systems stay visible with a warning marker so their lore and source data remain inspectable.</p></div>
</div>
...
.status-pill{display:inline-block;padding:2px 7px;border:1px solid var(--line);border-radius:999px;background:#172332;color:#d7e6f3;font-size:12px}.status-destroyed{border-color:#9a3a2f;background:#3a1818;color:#ffd6c9}.status-accessible{border-color:#315f44;background:#12251a;color:#b8f7c6}.status-hidden{opacity:.65}.system-map-wrap canvas{background:#06101a}.system-map-wrap{display:grid;gap:8px}.boss-layer-sprite,.boss-mini-sprite{transform:rotate(var(--base-rot,0deg)) scaleX(var(--mirror,1))}@keyframes boss-layer-spin{from{transform:rotate(var(--base-rot,0deg)) scaleX(var(--mirror,1))}to{transform:rotate(calc(var(--base-rot,0deg) + var(--spin,360deg))) scaleX(var(--mirror,1))}}
